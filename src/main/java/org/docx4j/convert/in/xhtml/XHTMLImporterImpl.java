...
public class XHTMLImporterImpl implements XHTMLImporter {
	...
	public XHTMLImporterImpl(WordprocessingMLPackage wordMLPackage) {
		...
		try {
			...
			if (ImportXHTMLProperties.getProperty("docx4j-ImportXHTML.Element.Heading.MapToStyle", false)) {
				headingHandler = new HeadingHandler(wordMLPackage.getMainDocumentPart().getStyleDefinitionsPart().getContents());
			}
		}
		catch (Docx4JException err) {
			throw new Docx4JRuntimeException(err.getMessage());
		}
	 ...
	 * @since 3.0
	 */
	private void setDefaultFontSize() throws Docx4JException {
		
		StyleDefinitionsPart sdp = wordMLPackage.getMainDocumentPart().getStyleDefinitionsPart();
		if (sdp!=null
			 && sdp.getContents().getDocDefaults()!=null) {
			
			RPrDefault rPrDefault = sdp.getContents().getDocDefaults().getRPrDefault();
    ...
    private void initStyleMap(StyleDefinitionsPart sdp) throws Docx4JException {
    	
    	if (sdp==null) return;

    	org.docx4j.wml.Styles styles = sdp.getContents();
